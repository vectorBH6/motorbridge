...
| RobStride | rs-00, rs-01, rs-02, rs-03, rs-04, rs-05, rs-06 | scan, ping, enable, disable, MIT, POS_VEL, VEL, parameter read/write, set-id, zero | Yes (i8/i16/i32/u8/u16/u32/f32/string raw chunks, model-specific) | Yes | Uses 29-bit extended CAN IDs; default host/feedback ID `0xFD`; `--model` must match the physical RS model because manual parameter tables differ |

RobStride note: high-level control commands are shared across RS00-RS06, but manual function-code parameters are model-specific. Parameter table data is aligned with RobStride/Product_Information commit `ba7236bc26417766fda71e75ae128c66dbd21aba`.
